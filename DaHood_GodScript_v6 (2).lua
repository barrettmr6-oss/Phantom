...
local function Notify(title, msg, t, col)
    ...
    -- slide-in from right using Position offset (UIListLayout controls Y)
    bg.AnchorPoint = Vector2.new(0,0)
    -- Use a wrapper approach: animate via BackgroundTransparency + size
    ...
    for _, lbl in ipairs(bg:GetDescendants()) do
        ...
        if lbl:IsA("UIStroke") then lbl.Transparency = 1 end
    end
    task.spawn(function()
        task.wait(0.02)
        TweenService:Create(bg,TweenInfo.new(0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{BackgroundTransparency=0}):Play()
        for _, lbl in ipairs(bg:GetDescendants()) do
            if lbl:IsA("TextLabel") then TweenService:Create(lbl,TweenInfo.new(0.25),{TextTransparency=0}):Play() end
            if lbl:IsA("Frame") and lbl.Name~="" then TweenService:Create(lbl,TweenInfo.new(0.25),{BackgroundTransparency=0}):Play() end
            if lbl:IsA("UIStroke") then TweenService:Create(lbl,TweenInfo.new(0.25),{Transparency=0}):Play() end
        end
    end)
    ...
    task.delay(t or 3, function()
        ...
        for _, lbl in ipairs(bg:GetDescendants()) do
            if lbl:IsA("TextLabel") then TweenService:Create(lbl,TweenInfo.new(0.25),{TextTransparency=1}):Play() end
            if lbl:IsA("Frame") then TweenService:Create(lbl,TweenInfo.new(0.25),{BackgroundTransparency=1}):Play() end
        end
...
local function StartAutoReload()
    ...
    S.AutoReloadConn = RunService.Heartbeat:Connect(function()
        ...
        local now = tick()
        if now - _reloadTick < 0.5 then return end
        _reloadTick = now
        pcall(function()
            local c=GetChar(); if not c then return end
...
local function StartKillAura()
    ...
    S.KillAuraConn = RunService.Heartbeat:Connect(function()
        ...
        local now = tick(); if now - _auraTick < 0.05 then return end; _auraTick = now
...
local function StartAutoFarm()
    ...
    S.AutoFarmConn = RunService.Heartbeat:Connect(function()
        ...
        local now = tick()
        if now - _farmTick < 0.8 then return end
        _farmTick = now
        pcall(function()
                ...
                if obj:IsA("BasePart") or obj:IsA("Model") then
                    ...
                    if n:find("cash") or n:find("money") or n:find("dollar") or n:find("drop") then
                        if (hrp.Position - obj.Position).Magnitude < 60 then
                            ...
                            if pickupRem then
                                pcall(function() pickupRem:FireServer(obj) end)
                            else
...
local function StatsBtn(txt,y,col,cb)
    local b=Instance.new("TextButton",StatsPanel); b.Size=UDim2.new(1,0,0,28); b.Position=UDim2.new(0,0,0,y)
...
StatsBtn("🎯 Target",140,C.Accent,function() S.AimbotOn=true; local t=GetAimbotTarget(); Notify("TARGET",t and "→ "..t.Name or "No target",2) end)
StatsBtn("☠️ Fling All",174,Color3.fromRGB(120,20,20),function()
    for _,pl in ipairs(Players:GetPlayers()) do
...
StatsBtn("⚡ Max Speed",208,Color3.fromRGB(30,60,80),function() S.SpeedVal=200; S.SpeedOn=true; Notify("SPEED","→ 200",2) end)
...
RunService.Heartbeat:Connect(function()
    pcall(function()
        ...
        local h=GetHum()
        if h then
            hpLbl.Text="❤️ "..math.floor(h.Health).."/"..math.floor(h.MaxHealth)
            -- update stamina label if a stamina value exists
            local stam = GetChar() and (GetChar():FindFirstChild("Stamina") or LP:FindFirstChild("Stamina"))
            if stam then stLbl.Text="⚡ "..math.floor(stam.Value) end
        end
    end)
end)
...
task.wait(1.5);  Notify("AC BYPASS","✅ Hooks active: CHECKER_1 + TeleportDetect + OneMoreTime",6,C.Yellow)
